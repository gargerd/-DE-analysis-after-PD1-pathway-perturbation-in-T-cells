...
```{r}
gen_exp<-read.csv(paste0(project,"_exp_data.txt"),header = TRUE,sep='\t')
rownames(gen_exp)<-gen_exp[,"ID_REF"]
gen_exp<-gen_exp[-1]
```

```{r}
#getting mouse mgi symbol names and saving them to file
# mouse <- useMart("ensembl", dataset = "mmusculus_gene_ensembl")
# g <- getBM( values=rownames(gen_exp), attributes=c("affy_mouse430a_2", "mgi_symbol"),mart = mouse)
# write.csv(g,"gene_names.csv")
```
...
summarized <-gene_names%>%
            group_by(affy_mouse430a_2) %>%
            dplyr::summarize(no=n_distinct(mgi_symbol))
filtered<-filter(summarized,no>1)
ids_to_exclude<-(rownames(gen_exp)%in% filtered$affy_mouse430a_2)
...
gen_exp_2<-data.frame(t(gen_exp))

pca_res <- prcomp((gen_exp_2), scale. = TRUE)
gen_exp_2$pd1_stim<-metad$PD1_STIMULATION
summary(pca_res)
autoplot(pca_res,data=gen_exp_2,colour="pd1_stim")
...
group <- factor(metad$PD1_STIMULATION, levels=c("y","n"))
design<-model.matrix(~group)
colnames(design) <- c("intercept","PD1 stimulation")

#fit model
fit <- lmFit(eset, design)
fit <-eBayes(fit)
tt<-topTable(fit, coef="PD1 stimulation", adjust="BH",number=100000)
...
gene_symbols<-gene_names[match(rownames(tt),gene_names$affy_mouse430a_2),]
tt$gene_name<-gene_symbols$mgi_symbol
